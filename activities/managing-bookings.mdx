...
## Create a booking from the calendar

To book a customer onto an existing session without going through the public booking page:

<Steps>
  <Step title="Open the session">
    Find the session on the [calendar view](/getting-started/calendar-view) and click it to open the session popup.
  </Step>
  <Step title="Start a new booking">
    Select **New booking** in the top right of the popup, then choose how many tickets to add.
  </Step>
  <Step title="Choose the customer">
    Search for an existing customer in your database, or create a new one by entering their name, email, and other required details on the spot.
  </Step>
  <Step title="Take payment">
    Pick a payment method:

    - **Card** — charge a card now (via [card reader](/point-of-sale/card-readers) or manual entry)
    - **Cash / other** — record an offline payment
    - **Invoice** — send an invoice for later payment (useful for corporate or group bookings)
    - **Defer** — confirm the booking now and collect payment later
  </Step>
  <Step title="Accept terms and finalise">
    Tick the terms and conditions box on the customer's behalf, then either **Add to basket** to keep selling or **Pay now** to complete the order.
  </Step>
</Steps>

<Tip>
  For walk-ins, food, drinks, retail, and any sale that isn't tied to a specific calendar session, use the [POS](/point-of-sale/overview) instead.
</Tip>
...
The confirmation email includes the current booking details and a PDF receipt attachment. If the recipient is the booking owner, a [calendar invite](/customers/notifications#calendar-invites) (`.ics`) is also attached. If the booking has been modified since the original confirmation, the resent email reflects the latest information.
