...
To record usage, send a POST request to the meter usage endpoint with the following body:
...
To retrieve aggregated usage, send a GET request to the meter usage endpoint with `from` and `to` query parameters in `YYYY-MM-DD` format:
...
When you query usage for a date range, built-in meters are included in the response:
