...
keywords: ['discount codes', 'promo codes', 'coupons', 'percentage discount', 'usage limits']
---
...
### Multiple eligible membership discounts

A customer may qualify for more than one membership discount on the same item — for example, when they hold multiple memberships, or when a single membership plan grants multiple discount benefits that all apply to the same product.

When this happens, Sailia automatically applies the **best** discount for the customer rather than the first one found. "Best" means the largest monetary reduction on the item after the discount is calculated:

- For **percentage** discounts, this is the percentage that produces the highest saving on the item's price.
- For **fixed** discounts, this is the largest fixed amount (capped by the item's price).
- Percentage and fixed discounts are compared directly by their monetary effect on the item.

Only one membership discount applies to a given item — they do not stack with each other. Non-membership discount codes continue to follow the normal [stacking behavior](#stacking-behavior).
...
## Baskets using this code

The discount details page includes a **Baskets using this code** table that lists every basket where the discount has been redeemed. Use it to review how a code is being used, troubleshoot customer queries, or audit redemptions.

Each row shows:

| Column | Description |
| --- | --- |
| **Amount** | The total amount the customer paid for the basket |
| **Status** | The current state of the basket — Pending, Successful, Failed, Unreconciled, Expired, or Void |
| **Customer** | The customer or family associated with the basket |

Select any row to open the full basket details, including line items, payment method, and refund history.

<Note>
  The table only lists baskets that successfully applied the discount. Baskets where the code was entered but rejected (for example, because a usage limit was reached) are not shown.
</Note>
