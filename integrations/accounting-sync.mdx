...
<Steps>
  ...
  <Step title="Assign categories to products">
    ...
    Add-ons have their own accounting category, separate from the activities they are linked to. Assign each add-on a category that reflects its own VAT treatment so equipment hire, photography, meal upgrades, and similar extras are reported under the correct account code rather than inheriting the parent activity's category.
