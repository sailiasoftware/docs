...
<Steps>
  ...
  <Step title="Map categories to Xero">
    ...
    - A **Xero tax type** (e.g. `OUTPUT2` for 20% VAT)
    - A **Xero account code** (e.g. a revenue account)
...
2. **Creates an invoice in Xero** — each payout becomes a single authorized invoice with line items grouped by VAT rate and account code.
3. **Marks the invoice as paid** — a payment is recorded against the appropriate clearing account (Stripe or cash).
4. **Attaches a CSV breakdown** — a detailed product-level breakdown is attached to each invoice for auditing.
