...
keywords: ['point of sale', 'POS', 'till', 'in-person sales', 'cash register']
---
...
When the POS has no active order, it displays a list of recent pending baskets for the current terminal. This helps staff quickly resume an in-progress sale instead of starting a new one.

Each card shows the customer name (or "Walk-in customer" if no account is linked), the basket total, and when it was last updated. Select a card to load that basket into the POS and continue where it left off.

<Note>
  Only baskets that still contain items and have not expired are shown. Up to ten recent pending baskets appear at a time.
</Note>

### Open a pending basket from its receipt

You can also resume a pending basket directly from the [bookings dashboard](/activities/managing-bookings#bookings-dashboard). Open the order receipt for the basket and select **View in POS** to load it into the point of sale.

The **View in POS** button only appears on receipts for baskets in a **Pending** state, and is available to staff with the **Operator Booking** permission. Once a basket has been paid or otherwise completed, the button is no longer shown.
...
Type the customer's email address or name into the search field. Matching accounts appear as you type, showing the account email and the primary account holder's name.

If a search matches someone in a family — for example, a child's name — the result still shows the main account, with a label like **Tom Owen + 4 more** indicating how many other people are on that account. Select the row to link the family account to the order; you can choose the specific family member after the account is selected.
...
- Long titles are truncated on the tile. Hover the tile to see the full title.
- Folders and category tiles also use custom titles for their display name.

### Tile meta captions

Each tile shows a short caption in the bottom-left corner that gives staff at-a-glance context without opening the item. The caption depends on the tile type:

| Tile type | Caption shown |
| --- | --- |
| **Shop product** (no variants) | Default price |
| **Shop product** (with variants) | Number of variants — for example, `3 variants` |
| **Activity** | Ticket price range — for example, `£5.00 – £12.00`. Shows a single price when all tickets are the same price. Upsell-only tickets are excluded. |
| **Membership** or **Activity pass** | Default price |
| **Gift card** | Default price |
| **Discount code** | Discount amount — `10%` for percentage codes, or the fixed amount for fixed-value codes |
| **Folder** | Folder icon |
| **Category** | `Category` label |

Captions update automatically when prices, variants, or discount values change on the underlying item — you don't need to edit POS tiles after pricing changes elsewhere in Sailia.
