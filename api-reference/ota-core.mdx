---
title: OTA core endpoints
description: Integrate with Sailia as an online travel agency using the Core OTA API.
keywords: ['OTA', 'online travel agency', 'availability', 'bookings', 'API']
---

The Core OTA API provides a standardized set of endpoints for online travel agency (OTA) partners to list activities, check availability, and create bookings in Sailia. Use these endpoints to build integrations that sell a Sailia operator's activities on your own platform.
...
All Core OTA endpoints are under:
...
curl -H "Authorization: Bearer YOUR_API_TOKEN" \
  "https://api.sailia.com/api/v2/ota/core/products?name=Sailing"
```
...
```
GET /api/v2/ota/core/resolve-product
```
...
```bash
curl -H "Authorization: Bearer YOUR_API_TOKEN" \
  "https://api.sailia.com/api/v2/ota/core/resolve-product?Name=Adventure%20Tour"
```
...
```
GET /api/v2/ota/core/availability/{ActivityID}/dates
```
...
```bash
curl -H "Authorization: Bearer YOUR_API_TOKEN" \
  "https://api.sailia.com/api/v2/ota/core/availability/s_abc123xyz456/dates?StartDate=2026-06-01&EndDate=2026-06-30"
```
...
```
GET /api/v2/ota/core/availability/{ActivityID}/timeslots
```
...
```bash
curl -H "Authorization: Bearer YOUR_API_TOKEN" \
  "https://api.sailia.com/api/v2/ota/core/availability/s_abc123xyz456/timeslots?Date=2026-06-15"
```
...
```bash
curl -H "Authorization: Bearer YOUR_API_TOKEN" \
  "https://api.sailia.com/api/v2/ota/core/questions/s_abc123xyz456"
```
...
curl -X POST \
  ...
  "https://api.sailia.com/api/v2/ota/core/booking"
```
